...
export default function ProductsTab() {
  ...
  const save = async () => {
    if (!form.name || !form.price) {
      setSaveError(ap.name_price_required);
      return;
    }
    setSaving(true);
    setSaveError(null);
    try {
      ...
      if (error) { logError('ProductsTab/save', error); setSaveError(error.message); showToast(error.message, 'error'); }
        ...
        logActivity({ action: editingProduct ? 'updated' : 'created', entity: 'products', entity_id: editingProduct?.id, label: editingProduct ? `${ap.product_updated}: ${form.name}` : `${ap.product_added}: ${form.name}` });
        resetForm(); invalidate(); showToast(editingProduct ? ap.product_updated : ap.product_added);
      }
    } catch (err: any) {
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <AnimatePresence>
        {deleteTarget && (
          <ConfirmModal
            message={`${ap.product_confirm_delete} "${deleteTarget.name}" ? ${ap.product_irreversible}`}
            ...
            >
              <div className="sticky top-0 bg-white dark:bg-slate-800 p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">
                  {editingProduct ? ap.product_edit : ap.product_add}
                </h3>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {[['name', ap.product_name + ' *'], ['category', ap.product_category], ['price', ap.product_price + ' *'], ['stock', ap.product_stock]].map(([k, label]) => (
                    <div key={k}>
