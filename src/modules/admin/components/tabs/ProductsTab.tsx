...
import { Plus, Trash2, RefreshCw, ShoppingBag, Edit } from 'lucide-react';
...
export default function ProductsTab() {
  ...
  const save = async () => {
    ...
    else { setShowForm(false); setForm({ name: '', description: '', price: '', category: '', image: '', stock: '0' }); fetch(); }
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {products.length === 0 ? (
          <div className="text-center py-12 text-slate-400"><ShoppingBag className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Aucun produit.</p></div>
        ) : products.map(p => (
          <div key={p.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 flex items-center gap-4">
            ...
            <button onClick={() => remove(p.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg transition-colors">
