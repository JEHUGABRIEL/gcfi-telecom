...
const STATUS_CONFIG = {
  nouveau:   { labelKey: 'quote_status_new' as const,   color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400',   icon: Clock },
  en_cours:  { labelKey: 'quote_status_in_progress' as const,  color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400', icon: RefreshCw },
  traité:    { labelKey: 'quote_status_processed' as const,    color: 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400', icon: CheckCircle },
  annulé:    { labelKey: 'quote_status_cancelled' as const,   color: 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400',         icon: XCircle },
};
...
export default function QuotesTab() {
  ...
  const getStatusLabel = (status: string) => {
    const key = STATUS_CONFIG[status as keyof typeof STATUS_CONFIG]?.labelKey;
    return key ? (ap as any)[key] : status;
  };
