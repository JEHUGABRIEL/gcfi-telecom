...
export default function NotificationCenter({
  onClose, notifications, onMarkAsRead, onClearAll,
      ...
      >
        ...
        <div className="shrink-0 flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {unreadCount > 0 && <p className="text-xs text-[var(--accent)] font-bold">{unreadCount} non lue{unreadCount > 1 ? 's' : ''}</p>}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto py-3 px-3 space-y-2 min-h-0">
          {notifications.length === 0 ? (
            ...
            >
              {!notif.read && <span className="absolute top-3 right-3 w-2 h-2 bg-[var(--accent)] rounded-full" />}
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-sm text-slate-900 dark:text-white mb-0.5 group-hover:text-[var(--accent)] transition-colors">{notif.title}</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{notif.message}</p>
                  <span className="text-[10px] text-slate-400 mt-1 block">
                    {new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' }).format(notif.timestamp)}
                  </span>
                </div>
              </div>
            </div>
