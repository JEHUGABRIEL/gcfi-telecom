...
serve(async (req: Request) => {
  ...
  // Accepte les appels internes Supabase (webhook) et les appels avec la clé service role
  const authHeader = req.headers.get("authorization") || "";
  const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";
  const isInternal = authHeader.includes(serviceRoleKey) || req.headers.get("x-supabase-webhook") === "true";

  if (!isInternal && !authHeader.startsWith("Bearer")) {
    // Autoriser si appelé depuis le webhook Supabase (pas de JWT requis)
